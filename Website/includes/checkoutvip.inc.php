...
if($spotNr=='none' && empty($SpotCode)){
  ...
  $amount= $balance;
  $sql = "INSERT INTO transaction(source,dateTimee,amount,ticketNr) VALUES('website','$date','$amount','$tknum');";
  mysqli_query($conn, $sql);
    ...
    if($resultCheck = 1){ //if code is correct
                         //need to find spotNr
   $sql2="SELECT spotNr FROM  camping_spot WHERE spotCode='$SpotCode'";
