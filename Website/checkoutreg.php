			 ...
			 <div class="row">
				 <div class="col">
					 <div class="home_content d-flex flex-row align-items-end justify-content-start">
						 <div class="current_page">Checkout</div>
...
<div>
  <form action="includes/checkoutreg.inc.php" method="POST" style="margin-top:80px;">
    <div class="row">
      <label class="fullname_signup">Balance:</label>
      <input type="text" name="balance" required="required" style="margin-left: 482px; margin-top:-30px; width: 40%; margin-bottom: 15px; font-size: 15px;">
    </div>
    <div class="SELECT">
    <label class="fullname_signup">Camping Spot €10/fee and €20/p:</label>
      <?php

$mysqli = new mysqli("studmysql01.fhict.local","dbi399963","1234567","dbi399963");
$sqlSelect="SELECT spotNr FROM camping_spot WHERE nrOfPersons='0'";
$result = $mysqli -> query ($sqlSelect);
echo "<select name='spotNr'>";

while ($row = mysqli_fetch_array($result)) {
    $rows[] = $row;
}
foreach ($rows as $row) {
print "<option value='" . $row['spotNr'] . "'>" . $row['spotNr'] . "</option>";
}
...
<div>
  <label class="fullname_signup">OR fill Spot Code Recevied by a friend:</label>
</div>
<div class="row">
  <label class="fullname_signup">Spot Code:</label>
  <input type="text" name="SpotCode"  style="margin-left: 482px; margin-top:-30px; width: 40%; margin-bottom: 15px; font-size: 15px;">
</div>
